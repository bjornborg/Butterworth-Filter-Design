...
TEST_CASE("Analogue lowpass prototype generation",
    " Verify poles match MATLAB buttap(8)")
{
  ...
  std::vector<std::complex<double>> baseline_poles;
  baseline_poles.resize(filterOrder);

  // Butterworth butter;
  std::vector<std::complex<double>> tempPoleStorage =
      Butterworth::prototypeAnalogLowPass(filterOrder);

  // copy tmppole into poles
  int index = 0;
  for (std::vector<std::complex<double>>::iterator itr =
           tempPoleStorage.begin();
       itr != tempPoleStorage.end(); itr++) {
    poles[index] = *itr;
    index++;
  }

  baseline_poles[0] = std::complex<double>(-0.19509032201613, 0.98078528040323);
  baseline_poles[1] =
      std::complex<double>(-0.19509032201613, -0.98078528040323);
  baseline_poles[2] = std::complex<double>(-0.55557023301960, 0.83146961230255);
  baseline_poles[3] =
      std::complex<double>(-0.55557023301960, -0.83146961230255);
  baseline_poles[4] = std::complex<double>(-0.83146961230255, 0.55557023301960);
  baseline_poles[5] =
      std::complex<double>(-0.83146961230255, -0.55557023301960);
  baseline_poles[6] = std::complex<double>(-0.98078528040323, 0.19509032201613);
  baseline_poles[7] =
      std::complex<double>(-0.98078528040323, -0.19509032201613);

  // Tolerance
  double const EPSILON = 1.0e-14;
    ...
    "Lowpass filter coefficients", " Verify coefficients for a lowpass filter")
{
  ...
  std::vector<Biquad> coeffs; // second-order sections (sos)
  Butterworth butterworth;

  bool designedCorrectly = butterworth.loPass(44100, // fs
      500, // freq1
      0, // freq2. N/A for lowpass
      filterOrder, coeffs, overallGain);
  ...
  // double a0 = 1.0;
  double a1 =
      -1.96762058043629 * (-1); // to convert to DF2T (direct form II transpose)
  double a2 =
      0.97261960500367 * (-1); // to convert to DF2T (direct form II transpose)
...
TEST_CASE("Highpass filter coefficients",
    " Verify coefficients for a highpass filter")
{
...
TEST_CASE("Parametric EQ coefficients",
    " Verify coefficients for a parametric filter")
{
  ...
  //            Design," J. Audio Eng. Soc., vol.53, pp. 1026-1046, November
  //            2005.
  //******************************************************************************
...
TEST_CASE(
    "Low shelf EQ coefficients", " Verify coefficients for a low shelf filter")
{
...
//-------------------------------------------------------------------------------------------------
TEST_CASE("High shelf EQ coefficients",
    " Verify coefficients for a high shelf filter")
{
...
static void read_write_file(char const *infilename, char const *outfilename)
{
...
TEST_CASE("Test gain control issue #3")
{
  ...
  char const *infilename = "sweep_0_20K.wav";
  char const *outfilename = "sweep_0_20K_filtered.wav";
