...
void Biquad::DF2TFourthOrderSection(double B0, double B1, double B2, double B3,
    double B4, double A0, double A1, double A2, double A3, double A4)
{
  ...
  a1 = (-A1) /
      A0; // The negation conforms the Direct-Form II Transposed discrete-time
  a2 = (-A2) / A0; // filter (DF2T) coefficients to the expectations of the
                   // process function.
  a3 = (-A3) / A0;
  a4 = (-A4) / A0;
}

void Biquad::DF2TBiquad(
...
void BiquadChain::processBiquad(double const *input, double *output,
    int32_t const stride, uint32_t const count, Biquad const *coeffs)
{
  ...
  for (uint32_t n = 0; n < count; n++) {
    ...
    yn[0] = coeffs[0].b0 * xn + coeffs[0].b1 * m_xn1 + coeffs[0].b2 * m_xn2 +
        coeffs[0].a1 * yn1[0] + coeffs[0].a2 * yn2[0];

    for (uint32_t i = 1; i < numFilters; i++) {
      yn[i] = coeffs[i].b0 * yn[i - 1] + coeffs[i].b1 * yn1[i - 1] +
          coeffs[i].b2 * yn2[i - 1] + coeffs[i].a1 * yn1[i] +
          coeffs[i].a2 * yn2[i];
    }
...
void BiquadChain::processFourthOrderSections(double const *input,
    double *output, int32_t const stride, uint32_t const count,
    Biquad const *coeffs)
{
  ...
  for (uint32_t n = 0; n < count; n++) {
    ...
    yn[0] = coeffs[0].b0 * xn + coeffs[0].b1 * m_xn1 + coeffs[0].b2 * m_xn2 +
        coeffs[0].b3 * m_xn3 + coeffs[0].b4 * m_xn4 +

        coeffs[0].a1 * yn1[0] + coeffs[0].a2 * yn2[0] + coeffs[0].a3 * yn3[0] +
        coeffs[0].a4 * yn4[0];

    for (uint32_t i = 1; i < numFilters; i++) {
      yn[i] = coeffs[i].b0 * yn[i - 1] + coeffs[i].b1 * yn1[i - 1] +
          coeffs[i].b2 * yn2[i - 1] + coeffs[i].b3 * yn3[i - 1] +
          coeffs[i].b4 * yn4[i - 1] +

          coeffs[i].a1 * yn1[i] + coeffs[i].a2 * yn2[i] +
          coeffs[i].a3 * yn3[i] + coeffs[i].a4 * yn4[i];
    }
